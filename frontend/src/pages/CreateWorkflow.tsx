        ...
        <Card padding="32px">
          <div style={{ display: 'flex', flexDirection: 'column', gap: '24px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label htmlFor="workflow-name" style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>Workflow name</label>
              <input 
                id="workflow-name"
                type="text" 
                value={name}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              <label htmlFor="workflow-description" style={{ fontSize: '14px', fontWeight: 600, color: 'var(--text)' }}>Current process description</label>
              <textarea 
                id="workflow-description"
                value={description}
                onChange={e => setDescription(e.target.value)}
                placeholder="Describe how this process is currently handled, business rules, approval chains, and external systems..." 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
              ...
              <div style={{ 
                ...
                <p style={{ margin: '0 0 16px 0', fontSize: '14px', color: 'var(--muted)' }}>Support PDF / DOCX standard operating procedures</p>
                <input 
                  type="file" 
                  id="file-upload" 
                  accept=".pdf,.docx"
          ...
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px', marginTop: '32px', paddingTop: '24px', borderTop: '1px solid var(--border)' }}>
            ...
            <Button onClick={handleAnalyze} disabled={isSubmitting || (!name && !file)}>
              {isSubmitting ? 'Creating...' : 'Analyze Workflow'}
            </Button>
          </div>
        </Card>
      </div>
