"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
exports.WorkflowGraphService = void 0;
const index_1 = require("../../db/index");
const GraphNormalizer_1 = require("../../domain/workflow-engine/GraphNormalizer");
const GraphValidator_1 = require("../../domain/workflow-engine/GraphValidator");
class WorkflowGraphService {
    static async getGraph(workflowId) {
        // 1. Get the active or latest version for this workflow
        const version = await index_1.db
            .selectFrom('workflow_versions')
            .select('id')
            .where('workflow_id', '=', workflowId)
            .orderBy('version', 'desc')
            .limit(1)
            .executeTakeFirst();
        if (!version) {
            throw new Error(`Workflow version not found for workflow: ${workflowId}`);
        }
        const versionId = version.id;
        // 2. Query nodes, edges, rules from the database
        const dbNodes = await index_1.db
            .selectFrom('workflow_nodes')
            .select(['id', 'kind', 'type', 'name'])
            .where('version_id', '=', versionId)
            .execute();
        const dbEdges = await index_1.db
            .selectFrom('workflow_edges')
            .select(['id', 'source_id', 'target_id', 'label', 'is_branch'])
            .where('version_id', '=', versionId)
            .execute();
        const dbRules = await index_1.db
            .selectFrom('business_rules')
            .select(['id', 'name', 'description', 'condition', 'node_id'])
            .where('version_id', '=', versionId)
            .execute();
        // 3. Normalize to domain models
        const { nodes, edges, rules } = GraphNormalizer_1.GraphNormalizer.normalize(dbNodes.map(n => ({ ...n, kind: n.kind || 'UNKNOWN', metadata: {} })), dbEdges, dbRules.map(r => ({ ...r, action: undefined, description: r.description || '', name: r.name || undefined, node_id: r.node_id || '' })));
        // 4. Validate graph integrity
        const validation = GraphValidator_1.GraphValidator.validate(nodes, edges);
        if (!validation.isValid) {
            // Depending on product requirements, we might still return the graph 
            // but with validation errors. For now, log them.
            console.warn('Graph validation failed:', validation.errors);
        }
        // 5. Translate Canonical Domain Model -> API DTO
        const dto = {
            nodes: nodes.map(n => ({
                id: n.id,
                label: n.label,
                kind: n.type, // Map Domain 'type' to API 'kind'
                type: n.automated ? 'automated' : 'human'
            })),
            edges: edges.map(e => ({
                id: e.id,
                source: e.sourceId,
                target: e.targetId,
                label: e.condition,
                isBranch: e.type === 'BRANCH'
            })),
            rules: rules.map(r => ({
                id: r.id,
                description: r.name || 'Rule',
                condition: r.condition,
                action: r.action,
                nodeId: r.nodeId || ''
            }))
        };
        return dto;
    }
}
exports.WorkflowGraphService = WorkflowGraphService;
