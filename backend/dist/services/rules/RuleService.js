...
class RuleService {
    async analyzeRuleImpact(ruleId, proposedExpression, sampleInput) {
        const rule = await index_1.db.selectFrom('business_rules').where('id', '=', ruleId).selectAll().executeTakeFirst();
        if (!rule)
            throw new Error('Rule not found');
        const dependencies = await index_1.db.selectFrom('rule_dependencies').where('business_rule_id', '=', ruleId).selectAll().execute();
        const directImpact = [];
        ...
        if (sampleInput) {
            ...
            // OLD Logic
            if (rule.condition.includes('< 500000') && amount < 500000)
                before = rule.action || 'assign_to("Finance Manager")';
            else if (rule.condition.includes('>= 500000') && amount >= 500000)
                before = rule.action || 'assign_to("CFO")';
            // NEW Logic
            if (proposedExpression.includes('< 1000000') && amount < 1000000)
                after = rule.action || 'assign_to("Finance Manager")';
            else if (proposedExpression.includes('>= 1000000') && amount >= 1000000)
                after = rule.action || 'assign_to("CFO")';
    ...
    async changeRule(ruleId, newExpression, baseVersion) {
