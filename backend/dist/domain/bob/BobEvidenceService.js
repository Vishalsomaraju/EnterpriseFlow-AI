...
exports.bobEvidenceService = exports.BobEvidenceService = void 0;
const db_1 = require("../../db");
const BobIngestionSchemas_1 = require("./BobIngestionSchemas");
const LifecycleOrchestrator_1 = require("../../services/build/LifecycleOrchestrator");
class BobEvidenceService {
    async processEvent(payload) {
        const data = BobIngestionSchemas_1.BobEventSchema.parse(payload);
    ...
    async processPlan(payload) {
        const data = BobIngestionSchemas_1.BobPlanSchema.parse(payload);
        await db_1.db.transaction().execute(async (trx) => {
    ...
    async processChanges(payload) {
        const data = BobIngestionSchemas_1.BobChangesSchema.parse(payload);
        ...
        if (shouldTriggerLifecycle) {
            ...
            LifecycleOrchestrator_1.lifecycleOrchestrator.onChangesReceived(data.build_id).catch(console.error);
        }
    }
    async processTestResults(payload) {
        const data = BobIngestionSchemas_1.BobTestResultSchema.parse(payload);
        let shouldTriggerLifecycle = false;
        await db_1.db.transaction().execute(async (trx) => {
        ...
        if (shouldTriggerLifecycle) {
            LifecycleOrchestrator_1.lifecycleOrchestrator.onTestsReceived(data.build_id).catch(console.error);
        }
    }
}
exports.BobEvidenceService = BobEvidenceService;
exports.bobEvidenceService = new BobEvidenceService();
